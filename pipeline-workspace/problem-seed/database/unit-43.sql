...
-- Unit: 식별 관계와 비식별 관계 (Unit ID: 43)

-- Lesson 생성
INSERT INTO lesson (id, unit_id, title)
VALUES (125, 43, '식별 관계와 비식별 관계 1'),
       (126, 43, '식별 관계와 비식별 관계 2'),
       (127, 43, '식별 관계와 비식별 관계 3');
...
VALUES (2463, 866, '부모의 PK가 자식의 일반 속성(FK)으로만 사용된다', '부모의 PK가 자식의 일반 속성(FK)으로만 사용되는 것은 비식별 관계이다. 식별 관계에서는 부모의 PK가 자식의 PK이자 FK로 사용된다.', false),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (2483, 873, '자식 엔터티가 독립적으로 존재할 수 있을 때', '자식이 독립적으로 존재할 수 있을 때는 비식별 관계를 선택한다.', false),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (2503, 880, '식별 관계', '회원이 탈퇴해도 게시글이 유지되어야 하므로, 부모와 자식의 생명주기가 분리되어야 한다. 이 경우 비식별 관계가 적절하다.', false),
       ...
       (2517, 884, '비식별 관계', '사원이 부서에 소속되지 않을 수도 있으므로, 부모 - 자식 관계가 선택적이다. 이런 경우 비식별 관계가 적절하며, 사원 테이블의 부서ID FK는 NULL을 허용할 수 있다.', true),
...
VALUES (253, 881, '실선', '식별 관계는 ERD에서 실선으로 표현된다. 실선은 부모와 자식 간의 강한 결합을 나타내며, 부모 데이터가 존재해야만 자식 데이터를 생성할 수 있음을 의미한다. 반면 비식별 관계는 점선으로 표현된다.'),
       (254, 885, '비식별,non-identifying,non identifying', '비식별 관계(Non-Identifying Relationship)에서는 부모 테이블의 기본 키를 자식 테이블의 외래 키로만 사용하고, 자식은 자체적인 기본 키를 가진다. ERD에서 점선으로 표현되며, 부모와 자식의 생명주기가 분리된다.');