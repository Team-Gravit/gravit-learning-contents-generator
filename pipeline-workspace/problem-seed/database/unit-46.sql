...
-- Unit: 외래키와 제약 조건 (Unit ID: 46)

-- Lesson 생성
INSERT INTO lesson (id, unit_id, title)
VALUES (134, 46, '외래키와 제약 조건 1'),
       (135, 46, '외래키와 제약 조건 2'),
       (136, 46, '외래키와 제약 조건 3');
...
VALUES (2643, 929, '자신의 테이블 내에서만 사용된다', '외래키는 다른 테이블의 기본키 또는 고유키를 참조하여 테이블 간 관계를 정의한다.', false),
       ...
       (2652, 932, '외래키 제약 위반', '외래키가 참조하는 부모 테이블에 해당 값이 존재하지 않으면 외래키 제약 위반이 발생한다. 부서ID ''999''가 부서 테이블에 없으므로 삽입이 실패한다.', true),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (2663, 936, '자식이 참조 중이면 부모 삭제 / 수정이 불가하다', '이것은 RESTRICT 옵션의 설명이다.', false),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (2683, 943, '해당 속성의 값이 중복되면 안 된다', '이것은 UNIQUE 제약의 설명이다.', false),
       ...
       (2689, 945, '해당 속성의 값이 특정 조건을 만족해야 한다', 'CHECK 제약은 해당 속성의 값이 특정 조건을 만족해야 함을 정의한다. 예를 들어 price > 0과 같은 조건을 지정할 수 있다.', true),
       ...
       (2693, 946, 'stock 값이 지정되지 않으면 0으로 설정된다', 'DEFAULT 제약은 INSERT 시 해당 컬럼의 값이 지정되지 않으면 기본값을 자동으로 설정한다. 여기서는 0이 기본값이다.', true),
