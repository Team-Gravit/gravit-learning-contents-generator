...
-- Unit: 데이터 모델링 기본 (Unit ID: 42)

-- Lesson 생성
INSERT INTO lesson (id, unit_id, title)
VALUES (122, 42, '데이터 모델링 기초 1'),
       (123, 42, '데이터 모델링 기초 2'),
       (124, 42, '데이터 모델링 기초 3');

-- Lesson 1 (id = 122)
INSERT INTO problem (id, lesson_id, instruction, content, problem_type)
VALUES (845, 122, '다음 중 데이터 모델링의 3단계 순서로 올바른 것은?', '데이터 모델링은 현실 세계의 데이터를 데이터베이스로 옮기기 위해 구조화하는 과정이다.', 'OBJECTIVE'),
...
VALUES (2403, 845, '물리적 → 논리적 → 개념적', '물리적 모델링은 마지막 단계이다. 데이터 모델링은 개념적 → 논리적 → 물리적 순서로 진행된다.', false),
       ...
       (2412, 848, '하나의 엔터티는 반드시 하나의 속성만 가져야 한다', '하나의 엔터티는 반드시 속성을 가져야 하지만, 하나만 가져야 하는 것은 아니다. 일반적으로 엔터티는 여러 개의 속성을 가진다.', true),
       ...
       (2419, 851, '일대일(1:1) 관계는 양쪽 엔터티가 여러 개와 대응된다', '일대일(1:1) 관계는 양쪽 엔터티가 각각 하나씩 대응되는 관계이다. 여러 개와 대응되는 것은 다대다(M:N) 관계이다.', false),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (2423, 852, 'E-R 다이어그램 작성', 'E-R 다이어그램 작성은 개념적 모델링 단계에서 수행한다. 논리적 모델링은 E-R 다이어그램을 DBMS에 맞게 매핑하는 단계이다.', false),
       (2424, 852, 'DBMS에 맞게 매핑 및 정규화 수행', '논리적 모델링은 개념적 모델링에서 만든 E-R 다이어그램을 DBMS에 맞게 매핑하고, 상세 속성을 추출하며, 정규화를 수행하는 단계이다.', true),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (2443, 859, 'DBMS에 맞게 매핑하는 단계이다', 'DBMS에 맞게 매핑하는 것은 논리적 모델링 단계이다. 개념적 모델링은 E-R 다이어그램을 작성하는 단계이다.', false),
       ...
       (2448, 860, '일대다(1:N)', '일대다(1:N) 관계는 한쪽 엔터티가 여러 개의 다른 엔터티와 대응되는 관계이다. 부서-사원 관계가 대표적인 예시로, 하나의 부서에 여러 사원이 속할 수 있다.', true),
...
INSERT INTO answer (id, problem_id, content, explanation)
VALUES (247, 861, '연결 엔터티,중간 테이블,연결 테이블,매핑 테이블,조인 테이블', '다대다(M:N) 관계는 물리적 데이터베이스에서 직접 구현할 수 없으므로, 연결 엔터티(중간 테이블)를 추가하여 일대다(1:N) 관계로 해소한다. 연결 엔터티는 양쪽 엔터티의 기본키를 외래키로 가지며, 이 두 외래키의 조합이 기본키가 된다.'),
