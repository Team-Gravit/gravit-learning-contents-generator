...
-- Unit: 관계형 모델 개념 (Unit ID: 44)

-- Lesson 생성
INSERT INTO lesson (id, unit_id, title)
VALUES (128, 44, '관계형 모델 개념 1'),
       (129, 44, '관계형 모델 개념 2'),
       (130, 44, '관계형 모델 개념 3');
...
VALUES (2523, 887, '데이터를 계층 구조로 저장한다', '계층 구조로 데이터를 저장하는 것은 계층형 데이터베이스이다. 관계형 데이터베이스는 2차원 표 형태로 데이터를 저장한다.', false),
...
VALUES (255, 888, '릴레이션,relation,테이블,table', '릴레이션(Relation) 은 관계형 데이터베이스에서 데이터를 2차원 표 형태로 표현한 것이다.물리적으로는 테이블(Table) 이라고도 한다. 구조를 정의하는 릴레이션 스키마와 실제 데이터인 릴레이션 인스턴스로 구성된다.'),
       (256, 891, '도메인,domain', ' 도메인(Domain) 은 하나의 속성이 가질 수 있는 원자값들의 집합이다.속성에 허용되는 값의 범위를 정의하며, 예를 들어 성별 속성의 도메인은 {남, 여}, 학년 속성의 도메인은 {1, 2, 3, 4}로 정의할 수 있다.');
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (2543, 894, '외래 키 값은 참조하는 테이블의 기본 키 값이어야 한다', '이것은 참조 무결성 제약에 대한 설명이다.', false),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (2563, 901, '올바르다', '릴레이션은 구조를 정의하는 릴레이션 스키마와 실제 데이터인 릴레이션 인스턴스로 구성된다. 스키마는 속성의 이름과 도메인을 정의하고, 인스턴스는 특정 시점의 실제 튜플 집합이다.', true),
