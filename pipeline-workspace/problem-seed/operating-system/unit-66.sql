...
VALUES (3811, 1343, 'Base Register는 프로세스 메모리 크기를 저장하여 보호 목적으로 사용된다', '프로세스 메모리 크기를 저장하여 보호 목적으로 사용되는 것은 Limit Register이다. Base Register는 시작 물리 주소를 저장한다.', false),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (3823, 1349, '페이지는 논리 메모리를 고정 크기로 나눈 블록이고, 프레임은 물리 메모리를 페이지와 같은 크기로 나눈 블록이다', '페이지는 논리 메모리를, 프레임은 물리 메모리를 동일한 고정 크기로 나눈 블록이다. 이는 페이징의 기본 개념이다.', false),
...
INSERT INTO option (id, problem_id, content, explanation, is_answer)
VALUES (3839, 1358, '페이징은 외부 단편화와 내부 단편화를 모두 완전히 해결한다', '페이징은 외부 단편화를 해결하지만 내부 단편화는 발생할 수 있다.', false),
