...
VALUES (147, 462, 'n-1,n - 1,(n-1)', 'n개의 정점을 가지는 그래프의 신장 트리는 n-1개의 간선으로 연결된다. 이는 트리의 기본 성질로, n개의 정점을 사이클 없이 모두 연결하는 데 필요한 최소 간선 개수가 n-1개이기 때문이다.'),
       (148, 465, '가중치,weight,웨이트,비용,cost', '최소 신장 트리는 간선 가중치의 합이 최소인 트리이다. 각 간선에 부여된 가중치(비용, 거리 등)의 총합을 최소화하는 것이 목표이며, 이를 통해 네트워크 설계 비용을 최소화할 수 있다.');
...
INSERT INTO answer (id, problem_id, content, explanation)
VALUES (149, 470, '정점,vertex,vertices,버텍스,노드,node', 'Prim 알고리즘은 정점(vertex)을 기준으로 신장 트리를 확장하는 알고리즘이다. 시작 정점에서 출발하여 MST 집합에 인접한 정점들 중 최소 가중치로 연결된 정점을 하나씩 추가하면서 트리를 확장한다.'),
       (150, 473, 'o(e log v),O(ElogV),o(elog v),O(E log V),o(elgv),O(ELgV)', 'Prim 알고리즘의 시간 복잡도는 우선순위 큐를 사용할 때 O(E log V)이다. 여기서 E는 간선의 개수, V는 정점의 개수를 의미한다. 각 간선에 대해 우선순위 큐 연산(log V)을 수행하기 때문이다.');
...
INSERT INTO answer (id, problem_id, content, explanation)
VALUES (151, 478, '간선,edge,가중치,weight,웨이트,에지', 'Kruskal 알고리즘은 간선(edge)을 기준으로 간선을 선택하는 알고리즘이다. 모든 간선을 가중치 오름차순으로 정렬한 후, 가중치가 작은 간선부터 순차적으로 선택하면서 MST를 구성한다.'),
       (152, 481, 'o(e log e),O(ElogE),o(elog e),O(E log E),o(elge),O(ElgE)', 'Kruskal 알고리즘의 시간 복잡도는 Union-Find 사용 시 O(E log E)이다. 간선 정렬에 O(E log E)가 소요되며, 이것이 전체 시간 복잡도를 결정한다. Union-Find 연산은 거의 O(1)에 가까운 시간에 수행된다.');