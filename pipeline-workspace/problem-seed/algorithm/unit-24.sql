...
VALUES (135, 414, '지역 최적,local optimum,최선의 선택,지역최적,로컬 옵티멈,국소 최적,로컬최적', '그리디 알고리즘은 각 단계에서 지역 최적(Local Optimum)을 선택하여 전역 최적을 기대한다. 이는 그리디 알고리즘의 핵심 전략으로, 매 순간 가장 좋아 보이는 선택을 한다.'),
       (136, 417, '최적 부분 구조,optimal substructure,최적부분구조,부분 구조의 최적성,최적부분구조 속성', '전체 문제의 최적해가 부분 문제의 최적해로 구성될 수 있는 경우를 최적 부분 구조라고 한다. 이는 그리디 알고리즘이 최적해를 보장하기 위해 필요한 두 가지 속성 중 하나이다.');
...
INSERT INTO answer (id, problem_id, content, explanation)
VALUES (137, 422, '6,6개,여섯,여섯 개,6 개,육', '500원 2개, 100원 2개, 50원 1개, 10원 1개로 총 6개의 동전이 필요하다. 그리디 알고리즘을 적용하여 큰 동전부터 최대한 사용하면 최소 개수를 구할 수 있다.'),
       (138, 425, '시작 시간 오름차순,시작 시간,시작시간,시작 시간의 오름차순,시작시간 오름차순', '회의실 배정 문제에서 모든 회의를 시작 시간 오름차순으로 정렬한 후 그리디 알고리즘을 적용한다. 이를 통해 가능한 많은 회의를 배정할 수 있다.');
...
INSERT INTO answer (id, problem_id, content, explanation)
VALUES (139, 430, '4,4개,넷,네 개,4 개,사', '5원 동전 4개를 사용하면 총 4개로 20원을 만들 수 있어 그리디의 5개(16원 1개, 1원 4개)보다 최적이다. 이는 그리디 알고리즘이 항상 최적해를 보장하지 않는다는 것을 보여주는 예시이다.'),
       (140, 433, '탐욕 선택 속성,greedy choice property,탐욕선택속성,그리디 선택 속성,탐욕적 선택 속성', '16원을 먼저 선택하는 것이 전체 최적해의 일부라고 보장할 수 없어 탐욕 선택 속성이 깨진다. 탐욕 선택 속성은 각 단계에서 최선의 선택을 했을 때 전체 문제의 최적해를 구할 수 있는 경우를 말한다.');