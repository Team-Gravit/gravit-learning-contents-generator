...
VALUES (129, 390, '스택,stack', 'DFS는 스택을 사용하여 구현하거나 재귀 함수로 구현한다. 재귀 함수도 내부적으로 호출 스택을 사용하므로 본질적으로는 스택을 활용하는 것이다. 스택의 LIFO(Last In First Out) 특성이 깊이 우선 탐색의 핵심 원리이다.'),
       (130, 393, '백트래킹,backtracking,back tracking', 'DFS는 한 방향으로 끝까지 탐색 후 다른 방향을 탐색하는 백트래킹 방식이다. 더 이상 갈 곳이 없으면 이전 정점으로 되돌아가서 다른 경로를 탐색하는 것이 백트래킹의 핵심 개념이다.');
...
INSERT INTO answer (id, problem_id, content, explanation)
VALUES (131, 398, '큐,queue', 'BFS는 큐(Queue)를 사용하여 구현한다. 먼저 들어간 노드를 먼저 처리하는 FIFO(First In First Out) 방식이 BFS의 레벨 순서 탐색을 가능하게 한다.'),
       (132, 401, '최단 경로,최단 거리,최단경로,최단거리,shortest path', 'BFS는 가중치가 없는 그래프에서 최단 경로를 보장한다. 가까운 노드부터 순차적으로 탐색하므로 처음 도착한 경로가 항상 최단 경로가 된다.');
...
INSERT INTO answer (id, problem_id, content, explanation)
VALUES (133, 406, 'bfs,너비 우선 탐색,breadth first search,breadth-first search', 'BFS는 가중치가 없는 그래프에서 최단 경로를 보장한다. 가까운 노드부터 순차적으로 탐색하므로 처음 도착한 경로가 항상 최단 경로가 된다.'),
       (134, 409, 'dfs,깊이 우선 탐색,depth first search,depth-first search', 'DFS는 현재 경로상의 노드만 기억하므로 메모리 사용량이 적다. BFS는 큐에 같은 레벨의 모든 노드를 저장할 수 있어 메모리 사용량이 더 많다.');