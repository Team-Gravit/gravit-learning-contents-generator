...
VALUES (993, 373, '최단 경로를 찾는 알고리즘이다', '최단 경로 탐색은 다익스트라 등의 알고리즘이므로 오답이다.', false),
       ...
       (1009, 379, 'DFS 방식과 BFS 방식 두 가지로 모두 구현 가능하다.', '스택을 이용한 DFS 방식과 진입 차수(Indegree) 큐를 이용한 BFS 방식이 모두 가능하다.', true),
       (1010, 379, '위상 정렬의 결과는 항상 유일하다.', '시작 노드(진입 차수 0)가 여러 개일 경우 다양한 정렬 결과가 나올 수 있다.', false),
       (1011, 379, '무방향 그래프(Undirected Graph)에서도 적용할 수 있다.', '위상 정렬은 반드시 방향성이 있고 사이클이 없는 그래프(DAG)에서만 가능하다.', false),
       (1012, 379, '사이클(Cycle)이 포함된 그래프에서도 모든 노드의 순서를 정렬할 수 있다.', '사이클이 존재하면 진입 차수가 0이 되지 않는 구간이 생겨 정렬을 완료할 수 없다.', false);

INSERT INTO answer (id, problem_id, content, explanation)
VALUES (125, 375, 'dag,사이클이 없는 방향 그래프,directed acyclic graph,비순환 방향 그래프', '위상 정렬은 DAG(Directed Acyclic Graph, 사이클이 없는 방향 그래프)에서만 수행 가능하다. 사이클이 존재하면 작업의 순서를 명확하게 결정할 수 없기 때문이다.'),
       (126, 377, 'kahn''s algorithm,칸 알고리즘,kahn algorithm,kahn,칸', '큐를 이용한 위상 정렬 알고리즘을 Kahn''s Algorithm(칸 알고리즘)이라고 한다. 진입 차수가 0인 노드를 큐에 넣고, 해당 노드와 연결된 간선을 제거하며 위상 정렬을 수행한다.'),
       (127, 380, '0,zero,제로', '위상 정렬을 수행할 때 처음에 큐에 삽입하는 노드는 진입 차수가 0인 노드이다. 진입 차수가 0이라는 것은 해당 노드가 다른 노드에 의존하지 않음을 의미하므로, 가장 먼저 수행할 수 있는 작업이다.');
...
INSERT INTO answer (id, problem_id, content, explanation)
VALUES (128, 385, '사이클,cycle,순환', '위상 정렬을 수행한 후 모든 노드를 방문하지 못했다면 그래프에 사이클이 존재한다는 의미이다. 사이클이 있으면 작업 간에 순환 의존성이 발생하여 올바른 순서를 결정할 수 없다.');