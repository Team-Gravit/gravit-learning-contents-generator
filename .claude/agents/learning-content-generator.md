...
4. **되풀이 self-check (작성 직후, SQL 구성 전 반드시 수행).** 6문제 각각에 대해 아래를 대조하고, 하나라도 걸리면 그 선지/본문을 고쳐 다시 통과시킨 뒤에만 다음 단계로 넘어간다.
...
3. **되풀이 self-check (Write 전 반드시 수행).** 재생성·조정한 problem의 본문·정답 선지에 mode="initial" 절차의 self-check(객관식·주관식·본문 삭제 테스트)를 적용하고, 걸리면 통과할 때까지 고친다.
4. 수정된 내용을 **lesson_sql_path**에 Write(덮어쓰기)
...
## 실패 처리
- 검증 스크립트가 non-zero 종료 시 **FAIL\n{stderr}** 반환 후 종료. 내부 재시도 없음. 호출자(Phase 3·5)의 3회 재시도로 처리한다.
