...
### Phase 5. 사용자 확인 (파일이 2개 이상인 경우)
동일 날짜 디렉토리에 여러 유닛의 산출물이 있을 때는 재생성·부분 실행 등으로 ID 범위가 섞여 있을 가능성이 있으므로 baseline 확정 전 사용자에게 확인한다.

제시 내용
- 스캔 대상: `{latest_date}`, 파일 수
- 파일별 추출값 표 (unit_id / lesson / problem / option / answer)
- Phase 4 집계 결과 (last_*_id)

사용자 응답 처리
- `확정` → Phase 6 정상 반환
- `수정 {값}` → 사용자가 제시한 값으로 덮어쓴 뒤 Phase 6 정상 반환
- `중단` → 실패로 보고
