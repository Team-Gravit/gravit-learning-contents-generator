...
3. `.claude/spec/pipeline/pipeline-state-template.md` 템플릿을 복사하여, `pipeline-workspace/pipeline-state-{오늘 날짜}-{seq}.md`를 추가한다.
...
7. **토큰 집계 마커 생성.** `touch pipeline-workspace/.tokmark-{오늘 날짜}-{seq}` 를 실행한다. 이 파일의 mtime이 실행 시작 시각이 되어, Phase 7에서 이 실행의 토큰 사용량을 집계하는 기준이 된다.

### 출력
- `pipeline-workspace/pipeline-state-{오늘 날짜}-{seq}.md` 생성
- `pipeline-workspace/.tokmark-{오늘 날짜}-{seq}` 생성 (토큰 집계 마커)
