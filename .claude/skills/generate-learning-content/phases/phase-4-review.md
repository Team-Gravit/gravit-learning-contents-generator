...
1. Phase 3 성공 유닛 중 skip되지 않은 유닛별로 `learning-content-reviewer` 서브에이전트를 병렬로 호출한다. 인자는 아래와 같다.
   - `lesson_path` → `pipeline-workspace/generation-output/{오늘 날짜}/{unit_id}/lesson.sql`
   - `output_path` → `pipeline-workspace/review-output/{오늘 날짜}/{unit_id}/review.md`
2. 각 유닛의 `review.md`의 생성 여부를 확인하고, 누락된 평가 항목이 있는지 검증한다.
