---
description: 학습 콘텐츠 INSERT SQL 쿼리 작성 템플릿 및 순서. 파이프라인은 `_staging` 테이블에 저장한다.
---
...
본문(content)에는 코드 블록·표·여러 줄 설명을 마크다운으로 넣을 수 있다. 저장은 PostgreSQL(**psql**)로 이뤄지므로 아래를 지켜라.
...
- 저장은 한 트랜잭션으로 묶여 실행되므로(**--single-transaction**), 따옴표 이스케이프가 한 군데라도 틀리면 전체 저장이 취소된다. 작성 후 `validate-lesson-sql.py`로 점검하라.
